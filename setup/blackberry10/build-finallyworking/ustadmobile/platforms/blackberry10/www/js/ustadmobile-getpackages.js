...
var rootUrl;
    ...
    */
    ...
    function onPackageListTransfer(){
        ...
        //alert("in onPackageListTransfer()"); //works
    }

    function beginPackageListTransfer(){
        //alert("BB10TEST: Cordova ready okay and in beginPackageListTransfer()"); //works
        /*var platform;
        if(navigator.userAgent.indexOf("Safari") !== -1 && navigator.userAgent.indexOf("BB10") !== -1){
            platform = "bb10";
        }*/ //Not needed, BB10 window.open('html','_self') fixed child web view issue.
        //alert("Platform is: " + platform);    //works
        ...
        alert("b.i.s : " + blackberry.io.sandbox);
        //alert("BB10: blackberry.io.sandbox = false;");

        //window.webkitRequestFileSystem(window.PERSISTENT, 0, gotRootListDirPackage, function(){alert("Something went wrong in getting the fileSystem."); debugLog("Something went wrong in getting file system, beginPackageListTransfer()");});
        
        requestFileSystem(LocalFileSystem.PERSISTENT, 7 * 1024 * 1024, gotRootListDirPackage, function(){alert("Something went wrong in getting the fileSystem."); debugLog("Something went wrong in getting file system, beginPackageListTransfer()");});
    }

function gotListFail2(){
    console.log("Something went wrong in gotList()");
}

function gotList2(entries){
    var i;
    console.log("Going to show all files and folders in root");
...
function failDirectoryReader2(){
    console.log("Error in failDirectoryReader()");
}

function successDirectoryReader2(entries){
    var i;
    console.log("Going to show all files and folders in file:///store/ustadmobileContent");
    console.log("total entries1: " + entries.length);
    for (i=0; i<entries.length; i++) {
        console.log(" --->" + entries[i].name);
    }
    ...
    */
    function gotRootListDirPackage(rootFS){
        ...
        //var dirEntry = new DirectoryEntry({fullPath: "file:///"});
        //var directoryReader = dirEntry.createReader();
        //directoryReader.readEntries(successDirectoryReader2,failDirectoryReader2);
        
        //var reader = rootFS.root.createReader();
        //reader.readEntries(gotList2, gotListFail2);
        
        //alert("BB10TEST: In gotRootListDirPackage");
    ...
    */
    ...
    function startFileDownload(fileToDownload, folderName, callback){
        ...
        if (folderName == ""){
            ...
            if(typeof fileFolder === 'undefined'){   //09122013
                ...
                if(fileFolder != "books" && currentFileName != "all_ustadpkg_html5.xml"){
                    ...
                    //BB10 test:
                        //filePathDownload = rootPath + "ustadmobileContent/" + folderName + "/" + currentFileName; //new changed version
            ...
            //filePathDownload = encodeURI(filePathDownload);
            //alert("filePathDownload: " + filePathDownload);
        }
        //alert("Current URL " + document.URL); //For BB10: local:///ustadmobile_*.html
        
        //filePathDownload = blackberry.io.sharedFolder + "/testicles.xml"
        //filePathDownload = "/accounts/test.xml";
        //filePathDownload = "file:///store/test.xml";
        //filePathDownload = encodeURI(filePathDownload);
        //filePathDownload = "file:///store/test.xml";
        alert("filePathDownload: " + filePathDownload);
        var localPath = rootUrl + '/boobies.xml';
        ...
        fileTransfer.download(
            uri,
            localPath,   //filePathDownload
            function(entry){
                                     $.mobile.loading('hide');
                    alert("BB10TEST: In success!");
                                            /*
