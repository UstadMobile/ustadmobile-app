...
(function( $, undefined ) {
	...
	$.mobile.loadPage = function( url, options ) {
		...
		if ( !( $.mobile.allowCrossDomainPages || path.isSameDomain( documentUrl, absUrl ) ) ) {
			...
			/*
			//Old changes:
			console.log("In APP jQuery mobile-1.3.2.js. $.ajax url: fileUrl: " + fileUrl);
			var parts = fileUrl.split("x-wmapp0://");
			fileUrl = parts[parts.length-1];
			console.log("In APP jQuery mobile-1.3.2.js. $.ajax url: fileUrl has been changed (for windows phone): " + fileUrl);
			*/
			
			//Start of Ustad Mobile changes
			/*
			console.log("In CONTENT jQuery mobile-1.3.2.js. $.ajax url: fileUrl: " + fileUrl);
			var parts = fileUrl.split("x-wmapp0://");
			fileUrl = parts[parts.length-1];
			console.log("In CONTENT jQuery mobile-1.3.2.js. $.ajax url: fileUrl has been changed (for windows phone): " + fileUrl);
			*/
			
			console.log("In jQuery mobile-1.3.2.js. $.ajax url: fileUrl: " + fileUrl);
			var parts = fileUrl.split("x-wmapp0:///www/");
			var fileUrl2 = parts[parts.length-1];
			if(fileUrl2.length < fileUrl.length){
				console.log("In jQuery mobile-1.3.2.js Accessing MENU PAGE.");
				console.log("In jQuery mobile-1.3.2.js $.ajax Accessing app's resources from the content.");
				fileUrl = "/www/" + fileUrl2;
				console.log("In jQuery mobile-1.3.2.js. $.ajax url: fileUrl (in app) has been changed (for windows phone): " + fileUrl);
			}else{//If content..Put check here. some clash with app open /change Page calls. Test it. 
				var parts4 = fileUrl.split("x-wmapp0://www/");
				var fileUrl5 = parts4[parts4.length-1];
				if( fileUrl5.length < fileUrl.length){
					console.log("In jQuery mobile-1.3.2.js ACCESSING APP's resources from the APP itself");
					fileUrl = "www/" + fileUrl5;
					console.log("In jQuery mobile-1.3.2.js. $.ajax url: fileUrl (in app) has been changed (for windows phone): " + fileUrl);
				}else{
					var parts2 = fileUrl.split("x-wmapp0://");
					var fileUrl3 = parts2[parts2.length -1];
					if(fileUrl3.length < fileUrl.length){
						//fileUrl3 = fileUrl3.replace(/\/\//g, '\/');
						//fileUrl = fileUrl3;
						console.log("In jQuery mobile-1.3.2.js Accessing FIRST PAGE.");
						fileUrl = "x-wmapp0:" + fileUrl3;
						console.log("In jQuery mobile-1.3.2.js  $.ajax Accessing isolated storage's resources");
						console.log("In jQuery mobile-1.3.2.js. $.ajax url: fileUrl (in content) has been changed (for windows phone): " + fileUrl);
					}//else it is not Windows Phone
					else{
						var parts3 = fileUrl.split("x-wmapp0:/www/");
						var fileUrl4 = parts3[parts3.length-1];
						if(fileUrl4.length < fileUrl.length){
							console.log("In jQuery mobile-1.3.2.js. Accessing MENU PAGE: changePage called from Content.");
							console.log("In jQuery mobile-1.3.2.js. $.ajax url: fileUrl (in content) is: " + fileUrl); 
							fileUrl = "/www/" + fileUrl4;
							console.log("In jQuery mobile-1.3.2.js. $.ajax url: fileUrl (in content) has been changed (for windows phone): " + fileUrl);
						}else{
							console.log("In jQuery mobile-1.3.2.js. Accessing changePage called from Content / Isolated Storage. No changes made.");
						}
					}
				}
			}
			...
			//console.log("In jQuery mobile-1.3.2.js. $.ajax url: fileUrl has been changed AGAIN (for windows phone): " + fileUrl);
			//End of Ustad Mobile changes.
