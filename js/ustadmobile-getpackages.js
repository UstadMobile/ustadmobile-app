...
UstadMobileDownloader.prototype = {
    ...
    startCourseDownloadById: function() {
        ...
        this.downloadByID(courseId, null);
    },
...
function beginPackageListTransfer() {
    if (navigator.userAgent.indexOf("Safari") !== -1 && navigator.userAgent.indexOf("BB10") !== -1) {
        ...
        //default:
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, gotRootListDirPackage, function() {
            if (unitTestFlag == false) {
                alert("Something went wrong in getting the fileSystem.");
            }
            debugLog("Something went wrong in getting file system, beginPackageListTransfer()");
        });
...
function onlistPackages(msg) {
    ...
    if (navigator.userAgent.indexOf("Safari") !== -1 && navigator.userAgent.indexOf("BB10") !== -1) {
        ...
        //BB10:
        window.webkitRequestFileSystem(window.PERSISTENT, 0, getXMLListFile, function() {
            alert("Something went wrong in getting file System for packages.");
            debugLog("Something went wrong on getting file system in onlistPackages(msg)");
        });

    } else {
        //original
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, getXMLListFile, function() {
            alert("Something went wrong in getting file System for packages.");
            debugLog("Something went wrong on getting file system in onlistPackages(msg)");
        });
    }
...
function getXMLListFile(fileSystem) {
    ...
    fileSystem.root.getDirectory(getDir, {create: true, exclusive: false}, function() {
        ...
        fileSystem.root.getFile(forxml, {create: false, exclusive: false}, gotXMLFileList, function() {
            alert("Something went wrong in getting file for XML List processing.");
            debugLog("Something went wrong on getting file at getXMLListFile(fileSystem)");
        });
    }, function() {
        debugLog("Creating list Dir unsuccess.");
        $.mobile.loading('hide');
        alert("Getting Packages List on to your device failed.");
    });
...
function gotXMLFileList(fileEntry) {
     ...
     */
    fileEntry.file(gotXMLListFile, function() {
        alert("Something went wrong in getting XML file (Package List)");
        debugLog("Something went wrong in getting XML Package List of function gotXMLFileList(fileEntry)");
    });
}
...
function onPackageTransfer() {
    document.addEventListener('deviceready', beginPackageTransfer, function() {
        buttonBOOLEAN = true;
        console.log("buttonBOOLEAN is set to true because of failure. Can try again..");
        alert("Something went wrong in checking Cordova ready.");
        debugLog("Something went wrong on deviceready at function: onPackageTransfer()");
    });
    if (navigator.userAgent.indexOf("TideSDK") !== -1) {
...
function beginTideSDKCourseDownload() {
    ...
    if ((destinationDir.exists() == false) && (destinationDir.createDirectory() == false)) {
        buttonBOOLEAN = true;
        console.log("TideSDK [Get course] buttonBOOLEAN is set to true because of failure. Can try again..");
...
function beginPackageTransfer() {
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, gotRootDirPackage, function() {
        buttonBOOLEAN = true;
        console.log("buttonBOOLEAN is set to true because of failure. Can try again..");
        alert("Something went wrong in getting File System of Package XML");
        debugLog("Something went wrong in beginPackageTransfer()");
    });
}
...
function gotRootDirPackage(fileSystem) {
    ...
    fileSystem.root.getDirectory(packageXMLDir, {create: true, exclusive: false}, function() {
        ...
        buttonBOOLEAN = true;
        console.log("buttonBOOLEAN is set to true because of failure. Can try again..");
        debugLog("Creating package XML Dir unsuccess.");
        $.mobile.loading('hide');
        alert("Unable to download package to your device and file system.");
    });
        ...
        if (typeof fileFolder === 'undefined') {
            ...
            if (fileFolder != "books" && currentFileName != "all_ustadpkg_html5.xml" && fileToDownload.indexOf(server) === -1) { //server = 78.47.197.237
                ...
                if (navigator.userAgent.indexOf("TideSDK") !== -1) {
                    ...
                    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function(fileSystem2) {
                        fileSystem2.root.getDirectory(getDir, {create: true, exclusive: false}, function() {
                            debugLog("Creation of folder: " + fileFolder + " in Course folder is a success.");
                        }, function() {
                            debugLog("Creation of folder: " + fileFolder + " in Course folder failed!");
                        });
                    }, function() {
                        console.log("Unable to get fileSystem in course sub folder creation");
                    }
                    );
...
//->
        ...
        window.webkitRequestFileSystem(window.PERSISTENT, 0, getXMLFile, function() {
            buttonBOOLEAN = true;
            console.log("buttonBOOLEAN is set to true because of failure. Can try again..");
            alert("Something went wrong in getting the file system of the package file. Internal Error.");
            debugLog("Something went wrong in readPackageFile(msg) ");
        }); // errorfilesystem (messages->en.js)

    } else { // Other devies (not blackberry 10)
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, getXMLFile, function() {
            buttonBOOLEAN = true;
            console.log("buttonBOOLEAN is set to true because of failure. Can try again..");
            alert("Something went wrong in getting the file system of the package file. Internal Error.");
            debugLog("Something went wrong in readPackageFile(msg) ");
...
function getXMLFile(fileSystem) {
    ...
    fileSystem.root.getDirectory(getDir, {create: true, exclusive: false}, function() {
        ...
        debugLog("forxml is: " + forxml);
        debugLog("GETTING THE XML!");
        fileSystem.root.getFile(forxml, {create: false, exclusive: false}, gotXMLFile, function() {
            alert("Something went wrong in getting the file XML Package ");
            debugLog("Something went wrong in getXMLFile(fileSystem) ");
        });
    }, function() {
        buttonBOOLEAN = true;
        console.log("buttonBOOLEAN is set to true because of failure. Can try again..");
        debugLog("Creating package Dir unsuccess.");
        $.mobile.loading('hide');
        alert("Getting Package file on to your device failed.");
    });
}
...
function gotXMLFile(fileEntry) {
    ...
    fileEntry.file(gotFile, function() {
        buttonBOOLEAN = true;
        console.log("buttonBOOLEAN is set to true because of failure. Can try again..");
        alert("Something went wrong in getting Package XML file");
        debugLog("Something went wrong in gotXMLFile(fileEntry)");
    });
}
